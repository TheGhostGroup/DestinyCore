...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    std::string const info = Trinity::StringFormat("Containing %u new and %u archived updates.",
        result.recent, result.archived);

    if (!result.updated)
        TC_LOG_INFO("sql.updates", ">> %s database is up-to-date! %s", DBUpdater<T>::GetTableName().c_str(), info.c_str());
    else
        TC_LOG_INFO("sql.updates", ">> Applied %zu %s. %s", result.updated, result.updated == 1 ? "query" : "queries", info.c_str());
    ...
    if (!exists(base))
    {
        switch (DBUpdater<T>::GetBaseLocationType())
        {
            ...
            case LOCATION_DOWNLOAD:
            {
                ...
                TC_LOG_ERROR("sql.updates", ">> File \"%s\" is missing, download it from \"https://github.com/jasongdove/LegionCore-7.3.5/releases\"" \
    ...
    Path const& path)
{
    ...
    // Execute sql file
    args.emplace_back("-e");
    args.emplace_back(Trinity::StringFormat("BEGIN; SOURCE %s; COMMIT;", path.generic_string()));
