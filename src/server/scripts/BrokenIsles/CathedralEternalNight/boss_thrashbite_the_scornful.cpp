...
{
    boss_thrashbite_the_scornful(Creature* creature) : BossAI(creature, DATA_THRASHBITE)
    {
...
// 237726
class spell_coen_scornful_gaze : public AuraScript
{
    PrepareAuraScript(spell_coen_scornful_gaze);
    ...
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_coen_scornful_gaze::AfterRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};

// 238484
class spell_coen_beguiling_bio : public AuraScript
{
    PrepareAuraScript(spell_coen_beguiling_bio)
    ...
    void Register() 
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_coen_beguiling_bio::OnPereodic, EFFECT_2, SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
void AddSC_boss_thrashbite_the_scornful()
{
    ...
    RegisterAuraScript(spell_coen_scornful_gaze);
    RegisterAuraScript(spell_coen_beguiling_bio);
}
